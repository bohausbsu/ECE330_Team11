...
//  GPIOE->MODER |= 0x55555555;
  ...
  *******************************************************************************************************/
  ...
  while (1)
  {
	  ...
	  RED_BRT = analog_value >> 4;

	  ADC1->SQR3 = 2; // select ADC channel 0, change to ADC channel 1 for potentiometer
	  	  HAL_Delay(1);
	  	  ADC1->CR2 |= (1 << 30);
	  	  while(!(ADC1->SR & 1<<1)) {}
	  	  analog_value = ADC1->DR;
	  	  GREEN_BRT = analog_value >> 4;


	  	ADC1->SQR3 = 3; // select ADC channel 3
	  HAL_Delay(1);
	  ADC1->CR2 |= (1 << 30);
	  while(!(ADC1->SR & 1<<1)) {}
	  analog_value = ADC1->DR;
	  BLUE_BRT = analog_value >> 4;
	  HAL_Delay(10);
