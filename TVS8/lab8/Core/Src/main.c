...
int main(void)
{
  ...
  int nextPos = 0; // next digit
  GPIOD->ODR = 1 << (nextPos % 8); // display initial target (pos 1) on LED

  while (1)
  {
	  // read switches
	  int inputC = (GPIOC->IDR >> 12) & 0xF;

	  // increment target digit if PC10 pressed
	  if (~(GPIOC->IDR >> 10) & 1) {
		  nextPos++;
		  // keep bits 15:12, replace 11:0
		  GPIOD->ODR = (GPIOD->ODR & 0xF000) | (1 << (nextPos % 8));
		  HAL_Delay(250);
	  }
	  ...
	  // PC11 status = GPIOC->IDR >> 11
	  // set digit at pos to inputC value if PC11 pressed
	  if ((~(GPIOC->IDR >> 11)) & 1) {
		  Seven_Segment_Digit(nextPos % 8, inputC);
		  HAL_Delay(500); // 0.5s delay to prevent spam
	  }

	  // set inputC value LEDs
	  GPIOD->ODR = (GPIOC->IDR & 0xF000) | (GPIOD->ODR & 0xFFF);
...
void Seven_Segment_Digit (unsigned char digit, unsigned char hex_char)
{
	...
	*******************************************************************************/
		...
		GPIOE->ODR = 0xFF00;
	int data = _7SEG[hex_char];
	int sel = ~(1 << digit) << 8;
	GPIOE->ODR = sel | (~data & 0x00FF);
...
*******************************************************************************/
	...
	for (digit=0 ; digit<8 ; digit++)
	{
		...
		int val = (HexValue >> (digit * 4)) & 0xF;
	   Seven_Segment_Digit(digit, val/*Replace 0 with correct HexValue characters*/);
	}
