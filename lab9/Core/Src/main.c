...
int main(void)
{
  ...
  // turned on AO4 for thermo, thus extra F
  GPIOA->MODER = 0xFFF;
  GPIOE->MODER |= 0x55555555; // Port E mode register - make E8 to E15 outputs

  /*** Configure ADC1 ***/
  /**** TODO: ENABLE ADC1 RCC CLOCK  ****/
  // Turn on ADC1 clock by forcing bit 8 of the RCC APB2ENR register to 1 while
  // keeping other bits unchanged
  RCC->APB2ENR |= (1 << 8);

  /**** TODO: TURN ON ADC1 ****/  // Turn on ADC1 by forcing bit 0 OF CR2 to 1 while keeping other
  // bits unchanged
  ADC1->CR2 |= (1 << 0);
//  ADC1->CR1 = 1 << 24;
  ...
  *******************************************************************************************************/
  ...
  while (1)
  {
	  ...
	  // tested for thermometer after checkoff
		  ADC1->SQR3 = 4; // select ADC channel 0, change to ADC channel 1 for potentiometer
		  HAL_Delay(1);
		  /**** TODO: START ADC1 CONVERSION ****/    // Start a conversion on ADC1 by
		  // forcing bit 30 in CR2 to 1 while keeping other bits unchanged
		  ADC1->CR2 |= (1 << 30);
